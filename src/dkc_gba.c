...
static const struct dkc_archetype arch[] = {
...
static const struct dkc_gba_levels dkc[] = {
...
void dkc_gba_levels(
    ...
    for (i = 0; i < size; i++) {
        ...
        const struct dkc_gba_levels *d = &dkc[i];
        ...
        if (gba_data(rom, romsize, &bp_data,   &bp_len, arch[a]. bp, 0, GBA_COMP_BIOS)
        ||  gba_data(rom, romsize, &raw_data, &raw_len, arch[a].raw, 0, GBA_COMP_BIOS)
        ||  gba_data(rom, romsize, &lay_data, &lay_len,  d->lay.loc, d->lay.ofs, d->lay.type)
        ) {
            goto next;
        }
        ...
        if (tileset) {
            ...
            sprintf(name, "%s Tiles", d->name);
        }
        else {
            strcpy(name, d->name);
        }

        if (gba_layout(&lay_data, raw_data, att_data, &width, &height, mode))
            goto next;

        if (mode) gba_split(lay_data, att_data, width*height*9);

        decode_palette(rgb, &rom[d->pal], 256);

        // (Number of 24x24 tiles) * (24*24 dots) * (RGBA)
        bitplane = malloc((lay_len / 2) * 576 * 4);
