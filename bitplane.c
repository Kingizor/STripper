...
static void colourfix_pre(uint8_t *rom, uint8_t *pal_data, int fix) {
...
static void colourfix_post(uint8_t *rgb, int fix) {
...
void assemble_screen(uint8_t *bitplane, unsigned int raw_len, unsigned int width, char dir[255], char name[255]) {
    ...
    */
    ...
    uint8_t *out = malloc(width * height * 32 * 8 * 4);
    unsigned int i, j, k;
...
void assemble_level(uint8_t *bitplane, uint8_t *rom, uint8_t *layout, unsigned int layout_len, int position_addr, int vert, int layout_size, int fix, char dir[255], char name[255]) {
    ...
    */
    ...
    uint8_t *group = malloc(width * height * 4);
    ...
    if (position_addr) {
        ...
        uint8_t *level = malloc(level_width * level_height * 4);
...
*/
    ...
    uint8_t a = 0, b = 0, run = 1, t1A = 0, t1B = 0, t1C = 0, t1D = 0, t1E = 0, t1F = 0, t20 = 0, t21 = 0;
    int c = 0, x = 0, y = 0;
    int origin[5];
    int recurse = 0; // Stack
    int jmp;
